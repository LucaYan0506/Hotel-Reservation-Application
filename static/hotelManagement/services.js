document.addEventListener('DOMContentLoaded', () =>{
    ...
    document.querySelector('.menu-option-container#hotel-configuration #sub-menu #paid-services').style.color = 'white';
    load();
})
...
function show_servicesForm(){
    document.querySelector('#table-container').style.display = 'none';   
    document.querySelector('#form-container').style.display = 'block';   
}

function hide_servicesForm(){
    if (typeof myWindow !== 'undefined')
        myWindow.close();
    location.reload();
}

function create_row(services,i){
/*
<tr>
    ...
    <td>
        <button class="btn" onclick="view_services(1)" style="border:solid 1px gray;">
            <i class="material-icons" style="vertical-align: text-top;font-size: 1rem;color: black;padding-right: 2px;">remove_red_eye</i>
            View
        </button>
        <button class="btn" onclick="edit_services(1)" style="color: #fff; background-color: #007bff; border-color: #007bff;">
            <i class="material-icons" style="vertical-align: text-top;font-size: 1rem;padding-right: 5px;">edit</i>
            Edit
        </button>
        <button class="btn" onclick="delete_services(1)" style="color: #fff; background-color: #dc3545; border-color: #dc3545;">
...
*/
    ...
    td3.innerHTML = `   
    <button class="btn" onclick="view_services(${services.pk})" style="border:solid 1px gray;">
        <i class="material-icons" style="vertical-align: text-top;font-size: 1rem;color: black;padding-right: 2px;">remove_red_eye</i>
        View
    </button>
    <button class="btn" onclick="edit_services(${services.pk})" style="color: #fff; background-color: #007bff; border-color: #007bff;">
        <i class="material-icons" style="vertical-align: text-top;font-size: 1rem;padding-right: 5px;">edit</i>
        Edit
    </button>
    <button class="btn" onclick="delete_services(${services.pk})" style="color: #fff; background-color: #dc3545; border-color: #dc3545;">
        <i class="material-icons" style="vertical-align: text-top;font-size: 1rem;padding-right: 2px;">delete</i>
        Delete
    </button>`;
...
function load(){
    ...
    //show services
    fetch(`/admin/services/info/?start=${start}&end=${end}&contain=${contain}`)
    .then(response => response.json())
    .then(data =>{
        let i = start;
        data.services.forEach(services => {
           create_row(services,i++);
        });
        ...
        nextBtn.disabled = start > data.total_services;
...
function view_services(pk){
    show_servicesForm();

    fetch(`/admin/services/info/?pk=${pk}`)
    .then(response => response.json())
    .then(services => {
        document.querySelector('#form-container #id_title').value = services.title;
        document.querySelector('#form-container #id_title').disabled = true;

        document.querySelector('#form-container #id_price_type').value = services.price_type;
        document.querySelector('#form-container #id_price_type').disabled = true;

        document.querySelector('#form-container #id_price').value = services.price;
        document.querySelector('#form-container #id_price').disabled = true;

        document.querySelector('#form-container #id_active').checked = services.active;
        document.querySelector('#form-container #id_active').disabled = true;

        editor.setData(services.description)
...
function edit_services(pk){
    ...
    document.querySelector('form').action = "/admin/services/update/"
...
function delete_services(pk){
    if (confirm('Are you sure to delete this services?')){
        fetch()
        window.location.replace(`/admin/services/delete/?pk=${pk}`)
    }
}
...
function validation(elem){
    ...
    .then(message => {
        if (message.Result == 'Succeed')
            location.reload('/admin/services/')
        else{
