...
document.addEventListener('DOMContentLoaded', () =>{
    ...
    //if hall_id is known, then apply this filter 
    if (hall_id != 'None'){
        document.querySelector('select#id_hall').selectedIndex = hall_id
        document.querySelectorAll('#custom-select-container')[1].querySelector(`input[value="${hall_id}"]`).click();
    }
...
*/
    ...
    td0.innerHTML = housekeeping.hall.value;
...
function load(){
    ...
    //show housekeeping
    fetch(`/admin/hall/housekeeping/info/?start=${start}&end=${end}&filter=${filter}`)
...
function view_housekeeping(pk){
    ...
    fetch(`/admin/hall/housekeeping/info/?pk=${pk}`)
    .then(response => response.json())
    .then(housekeeping => {
        document.querySelector('#form-container #id_hall').selectedIndex = housekeeping.hall.pk
        document.querySelector('#form-container #id_hall').disabled = true;
...
function edit_housekeeping(pk){
    ...
    document.querySelector('form').action = "/admin/hall/housekeeping/update/"
...
function delete_housekeeping(pk){
    if (confirm('Are you sure to delete this housekeeping?')){
        fetch()
        window.location.replace(`/admin/hall/housekeeping/delete/?pk=${pk}`)
    }
}
